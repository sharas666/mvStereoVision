...
For the absolute measurements we placed obstacles in the room starting from 8.4 meters in 0.6m steps. The results are visualized in the following figures \ref{fig:absolute-chart}:

\begin{figure}[H]
        \centering
        \includegraphics[width=10cm]{images/absolute-measurements.pdf}
        \caption{Barchart showing absolute differences in measurements using Disparity Map and Disto}
...
The final Outcomes are displayed in the following table \ref{table:absolute-results}:

\begin{table}[htbp]
	\begin{center}
	\begin{tabular}{|l|r|}
	\hline
	Minimum Value & -0.17 \\ \hline
	Maximum Value & 0.13 \\ \hline
	Mean Difference & 0.0307142857 \\ \hline
	\end{tabular}
	\end{center}
	\caption{Min, Max and Mean Values from the table}
	\label{table:absolute-results}
\end{table}


\subsection{Relative measurements}
For the relative test we measured the distance from our system to two different obstacles in different distances to each other. The difference of the two measured distances is our final value. The following chart shows the measurement differences between the disparity values and ones from the Disto \ref{fig:relative-chart}.
...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|r|}
	\hline
	Minimum Value & 0.006179 \\ \hline
	Maximum Value & 0.085643 \\ \hline
	Mean Difference & 0.036503 \\ \hline
	\end{tabular}
	\end{center}
	\caption{Min, Max and Mean Values from the table}
